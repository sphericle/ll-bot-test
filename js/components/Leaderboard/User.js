        ...
        <div class="player-container">
            <div class="player">
                <div class="copy-container">
                    ...
                    <Copy
                        v-if="!copied"
                        @click="copyURL('https://anylist.pages.dev/#/leaderboard/user/' + entry.user.toLowerCase().replaceAll(' ', '_')); copied = true"
                    ></Copy>
                    <Copied
                        v-if="copied"
                        @click="copyURL('https://anylist.pages.dev/#/leaderboard/user/' + entry.user.toLowerCase().replaceAll(' ', '_')); copied = true"
                ...
                <div class="pack-container" v-if="entry.userPacks.length > 0">
                    <a v-for="pack in entry.userPacks" class="pack" :style="{ 'background': rgbaBind(packColor(pack.difficulty), 0.2) }" :href="'https://anylist.pages.dev/#/packs/pack/' + pack.name.toLowerCase().replaceAll(' ', '_')">{{ pack.name }} (+{{ pack.score }})</a>
                </div>
